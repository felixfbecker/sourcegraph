...
func TestGetRepoRank(t *testing.T) {
	...
	svc := newService(&observation.TestContext, mockStore, nil, nil, gitserverClient, nil, conf.DefaultClient(), nil)
...
func TestGetRepoRankWithUserBoostedScores(t *testing.T) {
	...
	svc := newService(&observation.TestContext, mockStore, nil, nil, gitserverClient, nil, mockConfigQuerier, nil)
